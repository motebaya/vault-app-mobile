package com.vaultapp.presentation.screens.platform

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.vaultapp.data.cache.FaviconCache
import com.vaultapp.data.local.preferences.AppPreferences
import com.vaultapp.data.local.preferences.PlatformSortField
import com.vaultapp.domain.entity.Platform
import com.vaultapp.domain.repository.PlatformRepository
import com.vaultapp.presentation.components.ToastManager
...
class PlatformViewModel @Inject constructor(
    ...
    private val authRepository: com.vaultapp.domain.repository.AuthRepository,
    val faviconCache: FaviconCache,
    private val keystoreManager: com.vaultapp.security.keystore.KeystoreManager
) : ViewModel() {
    ...
    fun onDeletePlatformConfirm(pin: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (pinResult is com.vaultapp.domain.entity.VaultResult.Error) {
                    val errorMessage = when (pinResult.error) {
                        is com.vaultapp.domain.entity.VaultError.WrongPin -> "Incorrect PIN"
                        is com.vaultapp.domain.entity.VaultError.TooManyAttempts -> "Too many attempts. Please wait."
    ...
    fun onEditPlatformConfirm(name: String?, domain: String?, type: String?) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                when (result) {
                    is com.vaultapp.domain.entity.VaultResult.Success -> {
                        _uiState.update { 
                            it.copy(
                                isEditing = false,
                                showEditDialog = false,
                    ...
                    is com.vaultapp.domain.entity.VaultResult.Error -> {
                        val errorMessage = when (val error = result.error) {
                            is com.vaultapp.domain.entity.VaultError.ValidationError -> error.message
